...
export default ({ reviews = 34, rate = 3, starSize = '35px', starColor = '#ffc107', starColorDisabled = '#DEDFE0', starColorHover = '#f9de8d', onlyReading, onStarClick }) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <>
          ...
          <label onClick={() => onlyReading ? {} : onStarClick(6 - i)} for={`star-${6 - i}`}>★</label>
        </>
      );
    }
    return stars;
